...
picu_outliers <- covid_testing %>% 
  filter(clinic_name == "picu") %>% # filter to the clinic with the name "picu"
  mutate(total_tat = col_rec_tat + rec_ver_tat) %>% # calculate total turnaround time
  filter(total_tat > 10)    # filter only turnaround time greater than 10 hours
```
...
In the `covid_testing` dataset the turnaround time metrics are labeled `col_rec_tat` and `rec_var_tat`. Your goal is to select just these columns. Review the dplyr cheatsheet for additional information on select and helper functions. Using what we learned as well as the dplyr cheatsheet which of the following code snippets could you use to select these columns?
